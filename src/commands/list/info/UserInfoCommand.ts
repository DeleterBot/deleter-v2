...
import UserInfoCommandConfig from '@src/commands/list/info/resources/configs/UserInfoCommandConfig'
...
export default class UserInfoCommand extends BaseCommand {
  ...
  constructor() {
    super('@deleter.commands.list.info.UserInfoCommand', new UserInfoCommandConfig())
  }
  ...
  async execute(msg: DeleterCommandMessage, info: Info): Promise<CommandExecutionResult> {
    ...
    const parser = new StringPropertiesParser(),
      root = `${info.guild.lang.interface}.deleter.commands.list.info.command.user`
